...
import main.CONTROL_MESSAGE;
import main.MENU_MESSAGE;
...
public class team_msgControl implements team_msgControlInterface {
	...
	public team_msgControl(mainFrame mFrame, team_dataManagement team_dataMng){
	...
	*/
		...
		ArrayList<team> teamsList = team_dataMng.getTeams_GUI();
		
		
		
		mFrame.messageLoop(CONTROL_MESSAGE.TEAM_TXT_OPEN_OK, teamsList);
...
//		ids = this.compareST(id, selectMenu,selectsubMenu);
	...
	public ArrayList<team> compareST(int id, int selectMenu, int selectsubMenu) {
		...
		ArrayList<team> teamsList = team_dataMng.getTeams_GUI();
		team team = team_dataMng.getTeam(id);
		ArrayList<team> findList = new ArrayList<team>();
		
		
		if(selectMenu == 1){
			for(team t : teamsList){
				if( t.getGameNumber() >= team.getGameNumber() ){
					findList.add(t);
				}
			}
		}
		
		mFrame.messageLoop(CONTROL_MESSAGE.TEAM_SUB_OK, findList);
	...
	@Override
	public void deleteTeam(int inx) {
		// TODO Auto-generated method stub
		team_dataMng.deleteTeam(inx);
		mFrame.messageLoop(CONTROL_MESSAGE.TEAM_TXT_OPEN_OK, team_dataMng.getTeams_GUI());
	}

	@Override
	public void fileOpenTeams( String fileName ) throws Exception {
		// TODO Auto-generated method stub
